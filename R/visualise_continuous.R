...
# same for spatial subsampling ----------------------------------------------------------------

# merge data
dat_spec_div <- dat_deep_spec %>%
  pivot_longer(cols = everything(), 
               names_to = "start_age", 
               values_to = "diversity") %>% 
  mutate(start_age = as.double(start_age)) %>% 
  ...
  bind_rows(dat_spec %>% 
              filter(str_detect(metric, "divvy")) %>% 
              mutate(metric = str_remove_all(metric, "divvy"), 
                     metric = str_to_sentence(metric))) 
# visualise
plot_spec_div <- dat_spec_div %>%
  ggplot(aes(start_age, mean_div,
             colour = metric)) +
  geom_vline(xintercept = epoch_age,
             colour = "grey95",
